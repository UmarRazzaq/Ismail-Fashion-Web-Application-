                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table id="" class="table table-bordered" width="100%">
                                    ...
                                    try{
                                        $now = date('Y-m-d');
                                        $stmt = $conn->prepare("SELECT *, product_id, SUM(quantity) AS TotalQuantity FROM products p INNER JOIN details d ON p.id=d.product_id GROUP BY product_id ORDER BY SUM(quantity) ASC LIMIT 10");
                                        $stmt->execute();
                                        $total = 0;
                                        foreach($stmt as $row){
